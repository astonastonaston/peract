# Adapted from https://github.com/haosulab/ManiSkill/blob/main/mani_skill/envs/tasks/tabletop/peg_insertion_side.py
# This is the multi-camera PegInsertionSide-v1 env. It's useless for now
...
class PegInsertionSideEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def _default_sensor_configs(self):
        ...
        left_shoulder_pose = sapien_utils.look_at(eye=[0.2, -0.2, 0.3], target=[-0.1, 0, 0.1])
        right_shoulder_pose = sapien_utils.look_at(eye=[-0.4, -0.1, 0.3], target=[-0.1, 0, 0.1])
        return [CameraConfig("base_camera", pose, 128, 128, np.pi / 2, 0.01, 100),
            CameraConfig(
                "left_shoulder_camera",
                pose=left_shoulder_pose,
                width=128,
                height=128,
                fov=np.pi / 2,
                near=0.01,
